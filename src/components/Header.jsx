                  ...
                  <div className="flex space-x-4">
                    {navigation.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className={classNames(item.current ? 'bg-zinc-900 text-white' : 'text-gray-300 hover:bg-zinc-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium')}
                    ...
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                      {auth.user && (
                        <>
                          <Menu.Item>
                            {({ active }) => (
                              <Link href="/profile" className={classNames(active ? 'bg-zinc-100' : '', 'block px-4 py-2 text-sm text-gray-700')}>
                                Your Profile
                              </Link>
                            )}
                          </Menu.Item>
                      ...
                      {!auth.user && (
                        <Menu.Item>
                          {({ active }) => (
                            <Link href="/login" className={classNames(active ? 'bg-zinc-100' : '', 'block px-4 py-2 text-sm text-gray-700')}>
                              Login
                            </Link>
                          )}
                        </Menu.Item>
            ...
            <div className="space-y-1 px-2 pb-3 pt-2 flex flex-col items-start">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={classNames(item.current ? 'bg-zinc-900 text-white' : 'text-gray-300 hover:bg-zinc-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium')}
                  aria-current={item.current ? 'page' : undefined}
                >
                  {item.name}
                </Link>
